...
import op.tools.docx2wiki.UploadBmpInfo;
...
public class WikiTransThread extends Thread {
    ...
    public WikiTransThread(TextField ptxtWikiUrl ,TextField ptxtWikiUser, PasswordField ptxtWikiPsw,
    ...
    public void run() {
        try {
            if (!loginWiki(txtWikiUrl.getText(), txtWikiUser.getText(), txtWikiPsw.getText())) {
                return;
            }
            ...
            for (File f : _objectFile) {
                if (f.exists() && f.canRead()) {
                    ...
                    if (suffix.equals("doc") || suffix.equals("docx")) {
                        ...
                        wikiMsg("word文件:" + f.getName() + "处理完成");
                    }

                    if (suffix.equals("xls") || suffix.equals("xlsx")) {
                        wikiMsg("开始处理excel文件:" + f.getName());
                        ...
                        wikiMsg("excel文件:" + f.getName() + "处理完成");
                    }

                } else {
                    wikiMsg("文件异常：" + f.getName());
                }
            }
        } catch (Exception ex) {
